...
def _test_model(model, input_shape, output_sequence_length, french_vocab_size):
    ...
    assert model.input_shape == (
        None,
        *input_shape[1:],
    ), "Wrong input shape. Found input shape {} using parameter input_shape={}".format(
...
def test_tokenize(tokenize):
    ...
    assert tokenized_sentences == tokenizer.texts_to_sequences(
        sentences
    ), "Tokenizer returned and doesn't generate the same sentences as the tokenized sentences returned. "


def test_pad(pad):
    tokens = [[i for i in range(4)], [i for i in range(6)], [i for i in range(3)]]
    ...
    assert np.all(
        padded_tokens == true_padded_tokens
    ), "Pad returned the wrong results."

    padded_tokens_using_length = pad(tokens, 9)
    assert np.all(
        padded_tokens_using_length
        == np.concatenate((true_padded_tokens, np.full((3, 3), padding_id)), axis=1)
